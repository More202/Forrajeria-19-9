...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Inicializar búsqueda de productos y autocompletado
    ...
    if (buscarProductoInput) {
        ...
        document.addEventListener('click', function(event) {
            if (event.target !== buscarProductoInput && event.target !== sugerenciasContainer) {
                sugerenciasContainer.style.display = 'none';
            }
        });
    }

    // Cargar todos los productos para el autocompletado
...
function cargarTodosLosProductos() {
    fetch('/obtener_todos_productos/')
        ...
        .catch(error => console.error('Error al cargar productos para autocompletado:', error));
}
function mostrarCategorias(animal) {
    const categoriasMostrar = document.getElementById('categorias-mostrar');
    if (!categoriasMostrar) return;
    
    categoriasMostrar.innerHTML = '<h3>Categorías disponibles:</h3>';
...
function actualizarTotal() {
    ...
    for (let i = 0; i < filas.length; i++) {
        ...
        const descuento = parseInt(filas[i].cells[3].getElementsByTagName('select')[0].value);
        const subtotalConDescuento = calcularDescuento(precio, cantidad, descuento);
        
        filas[i].cells[4].getElementsByClassName('subtotal')[0].innerText = subtotalConDescuento.toFixed(2);
    ...
    const totalConDescuento = calcularDescuento(subtotal, 1, descuentoTotalPorcentaje);

    document.getElementById('subtotal-amount').innerText = subtotal.toFixed(2);
    document.getElementById('descuento-amount').innerText = (subtotal - totalConDescuento).toFixed(2);
